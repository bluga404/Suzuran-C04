...
final class ARFaceViewModel: NSObject, ObservableObject, ARSCNViewDelegate {
    ...
    /// Snapshots captured at 2Hz during the scan for continuous ML detection.
    @Published var activeSnapshot: ARSnapshot? = nil

    /// The final mesh geometry captured at the end of the scan for the SceneKit view.
    @Published var finalFaceGeometry: ARFaceGeometry? = nil

    private var lastAnalysisTime: CFAbsoluteTime = 0
    private let analysisInterval: TimeInterval = 0.5 // 2Hz
...
extension ARFaceViewModel: ARSessionDelegate {
    ...
    func session(_ session: ARSession, didUpdate anchors: [ARAnchor]) {
        ...
        // Continuous Analysis Snapshot at 2Hz
        let now = CFAbsoluteTimeGetCurrent()
        if now - lastAnalysisTime >= analysisInterval,
           let frame = session.currentFrame,
           !isCalibrating, lightingCondition == .good,
           !completedSegments.allSatisfy({ $0 }) { // Stop analysis when scan is done
            
            lastAnalysisTime = now
            let snapshot = ARSnapshot(
        ...
        // Final Mesh Capture when scan completes
        if completedSegments.allSatisfy({ $0 }), finalFaceGeometry == nil {
            let geom = faceAnchor.geometry
            DispatchQueue.main.async { [weak self] in
                self?.finalFaceGeometry = geom
            }
        }
    }

    func session(_ session: ARSession, didUpdate frame: ARFrame) {
        // Lighting estimate check at 5 hz
        let now = CFAbsoluteTimeGetCurrent()
